...
def training_function(script_args, training_args):
    ...
    s3_uri = 's3://sagemaker-us-west-2-526909565990/neuron_events2025_fhirfly/data/clinical_notes_with_qa_medications_labeled.csv'
    ...
    def create_conversation(sample):
        system_message = (
            "You are a clinical information extraction assistant. Extract structured information "
            "(conditions, medications, dosages, statuses) from the clinical note and reply with valid JSON."
        )
        assistant_target = ""
        try:
            if isinstance(sample.get("extracted_json"), (dict, list)):
                assistant_target = json.dumps(sample.get("extracted_json"), ensure_ascii=False)
            else:
                assistant_target = str(sample.get("extracted_json", ""))
        except Exception:
            assistant_target = str(sample.get("extracted_json", ""))


        return {"messages": [{"role": "system", "content": system_message}, 
            {"role": "user", "content": sample.get("note", "")}, 
